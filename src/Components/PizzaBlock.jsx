...
export default function PizzaBlock({ data }) {
  let [activeType, setActiveType] = React.useState(data.types[0]);
  let typeNames = ['тонкое', 'традиционное'];

  let [activeSize, setActiveSize] = React.useState(data.sizes[0]);
      ...
      <div className="pizza-block__selector">
        <ul>
          {typeNames.map((type, ind) => (
            <li
              key={type}
              onClick={() => {setActiveType(ind)}}
              className={cn({
                active: activeType === ind,
                disabled: !data.types.includes(ind)
              })}
            >
              {type}
            </li>
        ...
        <ul>
          {availableSizes.map((size, ind) => (
            <li
              key={size}
              onClick={() => {setActiveSize(ind)}}
              className={cn({
                active: activeSize === ind,
                disabled: !data.sizes.includes(size)
              })}
            >
              {size} см
            </li>
